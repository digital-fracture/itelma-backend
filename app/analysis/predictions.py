...
def infer_last_window_from_lists(
    ...
    *,
    ...
    if p_hyp_5m >= thr_h5:
        messages.append(f"Рост вероятности гипоксии (горизонт 5 мин) ({p_hyp_5m:.0%}).")
    if p_hyp_15m >= thr_h15:
        messages.append(f"Рост вероятности гипоксии (горизонт 15 мин) ({p_hyp_15m:.0%}).")
    if not messages:
...
def format_ctg_report(messages, details, thr_low=0.4, thr_high=0.7):
    ...
    lines.append(
        "_Минимум — грубый индикатор ямки; клиническая децелерация оценивается относительно базального._"
    )
    lines.append("")
